...
HFI <- as.numeric(HFI)
Dist_Water <- as.numeric(Dist_Water)
Forest <- as.numeric(Forest)

# Check for collinearity
covariates <- cbind(Elevation, HFI, Dist_Water, Forest)
cor(covariates)
```
...
fit_quad <- ppm(loc_ppp ~ Elevation + I(Elevation^2) + Dist_Water + I(Dist_Water^2) + Forest + I(Forest^2), data = DATA)
...
```{r}
plot(parres(fit_quad, 'Elevation'))
```

```{r}
plot(parres(fit_quad, 'Dist_Water'))
```

```{r}
plot(parres(fit_quad, 'Forest'))
```
