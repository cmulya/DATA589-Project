...
```{r}
# Estimate rho
# rho <- rhohat(loc_ppp, Forest)

# Plot the estimated rho function with x-axis limits
#plot(rho, main = "Estimated Spatial Dependence between Moose and Distance to Water", 
     #xlim = c(0, max(Forest)))
```
...
env <- envelope(loc_ppp, fun = Kinhom, rank = 1, nsim = 19, fix.n = T)

# Plot Ripley's K-function corrected for inhomogeneity with envelopes
plot(K_inhom, main = "Ripley's K-function Corrected for Inhomogeneity")
plot(env, add = TRUE)
```
...
```{r}
# Compute confidence envelopes
env <- envelope(loc_ppp, fun = pcf, rank = 1, nsim = 19, fix.n = T)


# Plot g-function corrected for inhomogeneity with envelopes
plot(g_func, main = "g-function Corrected for Inhomogeneity")
